...
; ProcLang Definitions

; Procedural definition of procedures (p. 79)
; proc? : SchemeVal -> Bool
(define proc?
  (lambda (val)
    (procedure? val)))
...
(define expval->proc
  (lambda (val)
    (cases expval val
      (proc-val (proc) proc)
      (else (printf "Expected value extractor error: ~s ~s" `proc val)))))
...
; Expected output: #(struct:num-val -100)
(define proctest
"let x = 200
in let f = proc (z) -(z,x)
in let x = 100
